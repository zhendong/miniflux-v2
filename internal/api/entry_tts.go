...
// getTTSAudioHandler generates or retrieves cached TTS audio for an entry.
func (h *handler) getTTSAudioHandler(w http.ResponseWriter, r *http.Request) {
...
// serveTTSAudioFileHandler serves the cached TTS audio file.
func (h *handler) serveTTSAudioFileHandler(w http.ResponseWriter, r *http.Request) {
	// Check if TTS is enabled
	if !config.Opts.TTSEnabled() {
		response.JSONForbidden(w, r)
		return
